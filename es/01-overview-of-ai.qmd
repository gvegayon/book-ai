...
Lo experimenté directamente. Un usuario del paquete `netdiffuseR` de R [@R-netdiffuseR; @valente2020diffusion] me escribió diciéndome que ChatGPT le había informado sobre un algoritmo publicado por ciertos autores y afirmaba que mi paquete incluía una viñeta que lo demostraba. El usuario no podía encontrarla. La razón era simple: no existía. El modelo había fabricado una referencia plausible pero falsa.
...
Con la introducción de frameworks como el Model Context Protocol (MCP) de Anthropic [@anthropic2024mcp], anunciado en noviembre de 2024, los LLMs adquirieron la capacidad de moverse más allá de la ventana de chat. En lugar de simplemente proponer código, un agente podía ejecutarlo, observar el resultado e iterar si fallaba. La idea es directa pero poderosa: darle al modelo una herramienta para probar su propio resultado.

::: {.callout-note title="La importancia de los MCPs"}
La tecnología *Model Context Protocol* (MPCs) marcó un avance importante en la era de la IA. Luego del anuncio, las otras grandes compañias de IA--OpenAI [@wiggersOpenAIAdoptsRival2025], Google [@AnnouncingOfficialMCP] y Microsoft [@koesmarnoDataverseMCPServer2025] --adoptaron la tecnoligía de manera casi inmediata. Anthropic terminó donando la tecnología a Linux Foundation's Agentic AI Foundation [@foundationLinuxFoundationAnnounces2025].
:::
...
Este capítulo presentó dos conceptos fundamentales: cómo funcionan los Large Language Models prediciendo el siguiente token a partir del contexto, y cómo la IA agéntica amplía sus capacidades al conectarlos con herramientas y entornos. La siguiente tabla ofrece recomendaciones prácticas que refuerzan estas ideas.

| Objetivo | Hacer | No hacer |
|---|---|---|
| **Proporcionar contexto para obtener mejores respuestas** | Establecer un rol e incluir detalles relevantes: *"Eres un estadístico experto especializado en análisis de supervivencia. Necesito ayuda para interpretar una curva de Kaplan-Meier de un ensayo clínico con 200 participantes."* | Enviar una solicitud vaga sin contexto: *"Explica estos datos."* |
| **Equilibrar precisión y creatividad** | Adaptar el estilo del prompt al objetivo. Para respuestas factuales: *"Describe los supuestos de un modelo de riesgos proporcionales de Cox."* Para lluvia de ideas: *"Sugiere cinco enfoques no convencionales para estudiar la movilidad urbana."* | Usar el mismo estilo genérico de prompt para todas las tareas, sin importar si se necesita precisión o exploración creativa. |
| **Verificar las afirmaciones generadas por IA** | Pedir referencias verificables: *"Enumera artículos recientes sobre difusión en redes y proporciona DOIs para que pueda verificarlos."* Luego confirmar que las referencias existen. | Confiar en el resultado sin comprobarlo: *"Dime las referencias clave en difusión en redes"* y citarlas sin verificación. |
| **Usar IA agéntica para tareas de programación** | Permitir que el agente ejecute e itere: *"Escribe un script en Python que cargue `data.csv`, calcule estadísticas descriptivas y ejecútalo. Corrige cualquier error."* | Copiar y pegar código generado sin probarlo: *"Escribe un script en Python para mi análisis"* y usarlo directamente en tu proyecto. |
