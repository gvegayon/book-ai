...
With the introduction of frameworks such as Anthropic's Model Context Protocol (MCP) [@anthropic2024mcp], announced in November 2024, LLMs gained the ability to move beyond the chat window. Instead of merely proposing code, an agent could execute it, observe the result, and iterate if it failed. The idea is straightforward but powerful: give the model a tool to test its own output.

::: {.callout-note title="The importance of MCPs"}
The Model Context Protocol (MPCs) technology marked an important development in the AI era. After their announcement, the other large players--OpenAI [@wiggersOpenAIAdoptsRival2025], Google [@AnnouncingOfficialMCP] and Microsoft [@koesmarnoDataverseMCPServer2025]--immediately adopted the new technology. Anthropic later donated the technology to the Linux Foundation's Agentic AI Foundation [@foundationLinuxFoundationAnnounces2025].
:::
...
This chapter introduced two foundational concepts: how Large Language Models work by predicting the next token based on context, and how agentic AI extends their capabilities by connecting them to tools and environments. The following table offers practical recommendations that reinforce these ideas.

| Goal | Do | Don't |
|---|---|---|
| **Provide context to get better responses** | Set a role and include relevant details: *"You are an expert statistician specializing in survival analysis. I need help interpreting a Kaplan-Meier curve from a clinical trial with 200 participants."* | Send a vague request with no background: *"Explain this data."* |
| **Balance precision and creativity** | Match your prompt style to your goal. For factual answers: *"Describe the assumptions of a Cox proportional hazards model."* For brainstorming: *"Suggest five unconventional approaches to study urban mobility."* | Use the same generic prompting style for every task, regardless of whether you need accuracy or creative exploration. |
| **Verify AI-generated claims** | Ask for verifiable references: *"List recent papers on network diffusion and provide DOIs so I can check them."* Then confirm the references exist. | Trust the output at face value: *"Tell me the key references in network diffusion"* and cite them without verification. |
| **Use agentic AI for coding tasks** | Let the agent execute and iterate: *"Write a Python script that loads `data.csv`, computes summary statistics. Ensure it runs properly."* | Copy-paste generated code without testing: *"Write a Python script for my analysis"* and use it directly in your project. |
